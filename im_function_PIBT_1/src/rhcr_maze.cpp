...
RHCR_maze::RHCR_maze(int seed, int num_of_robots,
                     int env_id, vector<vector<int>> py_map, std::string project_path) : 
                     RHCR_class_pibt_learn(seed, num_of_robots, py_map.size(), py_map[0].size(), env_id, project_path)
{
...
void RHCR_maze::initialize_start_locations()
{
    ...
    {
        int loc;
        while(( loc = G->get_random_travel()) != -1 && !used[loc])   
        {
            int orientation = -1;
