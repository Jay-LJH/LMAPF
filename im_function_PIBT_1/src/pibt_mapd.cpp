...
bool PIBT_MAPD::run(const vector<vector<int>>& action_guide)
{
    ...
    for (auto a : A) {
        if (a->v_next.location == -1)              // if the agent has next location, then skip
            funcPIBT(a);// determine its next location
        }
    // acting
    // update plan
    for (auto a : A)
...
bool PIBT_MAPD::funcPIBT(Agent* ai, Agent* aj)
{
  // compare two nodes
    auto node_compare = [&](const State& v, const State& u) {
        ...
        if (action_guide[curr_ag_id][v_action] != action_guide[curr_ag_id][u_action]) return action_guide[curr_ag_id][v_action] > action_guide[curr_ag_id][u_action];
...
//    cout<<"agent"<<ai->id<<"sort order is "<<endl;
//    for (auto c : C)
//    {
//        cout<<myMap[c.location-ai->v_now.location]<<endl;
//    }
//    cout<<"action guide is "<<endl;
//    for (int i=0;i<5;i++)
//    {
//        cout<<action_guide[curr_ag_id][i]<<endl;
//    }
