...
# modify from CO_mapf_gym_select.py
class CO_MAPFEnv(gym.Env):
    ...
    def __init__(self,env_id,file_name=None):
        ...
        self.episode_len=TrainingParameters.N_MAX_STEPS   
        self.induct_value = -3
        self.eject_value = -2
        self.obstacle_value = -1
        self.travel_value = 0
        self.env_id=env_id
        # select vertex using REGION_GAP
        self.selected_vertex = []
        for i in range(0,self.world_high,CopParameters.REGION_GAP):
            for j in range(0,self.world_wide,CopParameters.REGION_GAP):
                if self.total_map[i, j] == self.travel_value:
                    self.selected_vertex.append((i, j))
        setattr(CopParameters,"FOV",CopParameters.FOV+CopParameters.ADDITIONAL_FOV)
        self.num_select_node=len(self.selected_vertex)
    ...
    def build_guide_map(self):
        ...
        for i in range(self.num_select_node):
    ...
    def joint_move(self,map_action):
        ...
        for i in range(self.num_agents):
            ...
            for dx in range(-1, 2): 
                for dy in range(-1, 2):
                    if (ax + dx, ay + dy) in self.selected_vertex:
                        node_index = self.node_index_dict[(ax + dx, ay + dy)]
